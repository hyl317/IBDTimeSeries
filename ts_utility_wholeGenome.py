...
map_positions = [chrom_positions[0], chrom_positions[1]]
...
print(f'length of map_positions: {len(map_positions)}')
...
for ch in np.arange(1,23):
    ...
    #ratemap = msprime.RateMap.read_hapmap(f'/mnt/archgen/users/yilei/Data/Hapmap/hapmap_min_map/min_map-0.05-{ch}.txt')
    ch2map[ch] = ratemap
...
def simulate_wholeGenome(demography, nSamples=[], gaps=[], minLen=4.0, record_full_arg=False, \
    ...
    for bp in breakpointsbp:
        chrom = 1 + np.searchsorted(chrom_positions_hapmap, bp)
        ...
        bp_ = bp
        if chrom > 1:
            bp_ -= chrom_positions_hapmap[chrom-2]
    ...
    if len(nSamples_copy) == 1:
        ret, results4IBDNe = ibd_segments(ts, bp2chAndCM, within=list(range(0, 2*nSamples_copy[0])), max_time=endTime, minLen=minLen, uniform_map=uniform_map)
        results.update(ret)
        ...
        for i in range(len(nSamples_copy)):
            ...
            for j in range(i, len(nSamples_copy)):
                ...
                if i == j:
                    ret, _ = ibd_segments(ts, bp2chAndCM, within=list(range(start_i, end_i)), max_time=endTime, minLen=minLen, uniform_map=uniform_map)
                    results.update(ret)
                else:
                    results.update(ibd_segments(ts, bp2chAndCM, \
                        between=[list(range(start_i, end_i)), list(range(start_j, end_j))], \
                            max_time=endTime, minLen=minLen, uniform_map=uniform_map))
        print(f'extracting IBD done, takes {round(time.time() - t1)}s')
        return results
    
def write2vcf_wholeGenome(ts, outFolder, random_seed=1, mutation_rate=1.25e-8):
    ...
    for ch in range(1, 23):
        t1 = time.time()
        mts_ch = mts.keep_intervals(np.array([map_positions[2*(ch-1)], map_positions[2*ch-1]]).reshape(1,2))
        ...
        lst = lambda x : x - map_positions[2*(ch-1)]
